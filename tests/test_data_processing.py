...
def test_process_data_empty_dict_becomes_na():
    """Empty dict fields should become N/A."""
    raw = _wrap([{"id": "u1", "name": "Dave", "division": {}}])
    result = app.process_data(raw, None)
    assert result[0]["division"] == "N/A"


def test_process_data_schema_simplified():
    """The 'schema' field should be simplified to its 'name' subfield."""
    raw = _wrap([{"id": "c1", "schema": {"name": "contact_v2", "id": "abc"}}])
    result = app.process_data(raw, None)
    assert result[0]["schema"] == "contact_v2"


def test_process_data_external_organization_simplified():
    """The 'external_organization' field should be simplified to its 'name'."""
    raw = _wrap([{"id": "c1", "external_organization": {"name": "Acme Corp", "id": "org1"}}])
    result = app.process_data(raw, None)
    assert result[0]["external_organization"] == "Acme Corp"


def test_process_data_integration_type_simplified():
    """The 'integration_type' field should be simplified to its 'id'."""
    raw = _wrap([{"id": "i1", "name": "Salesforce", "integration_type": {"id": "salesforce-crm", "name": "Salesforce CRM"}}])
    result = app.process_data(raw, None)
    assert result[0]["integration_type"] == "salesforce-crm"


def test_process_data_multiple_pages():
    """Multiple pages of entities should all be flattened."""
    raw = [
        {"entities": [{"id": "u1", "name": "Alice"}]},
        {"entities": [{"id": "u2", "name": "Bob"}]},
    ]
    result = app.process_data(raw, None)
    assert len(result) == 2
    assert result[0]["name"] == "Alice"
...
def test_process_data_empty_string_becomes_na():
    """Empty string fields should become N/A."""
    raw = _wrap([{"id": "u1", "name": ""}])
    result = app.process_data(raw, None)
    assert result[0]["name"] == "N/A"


def test_process_data_queues_empty_list():
    """Empty queues list should produce N/A."""
    raw = _wrap([{"id": "u1", "name": "Eve", "queues": []}])
    result = app.process_data(raw, None)
    assert result[0]["queues"] == "N/A"
